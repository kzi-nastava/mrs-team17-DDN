...
@Component({
  selector: 'app-admin-drivers-ride-history',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './admin-drivers-ride-history.html',
  styleUrl: './admin-drivers-ride-history.css',
})
export class AdminDriversRideHistory implements OnInit, OnDestroy {
  private http = inject(HttpClient);
  private baseUrl = inject(API_BASE_URL);

  drivers: AdminUserOptionDto[] = [];
  ...
  loadDrivers(): void {
    this.http
      .get<AdminUserOptionDto[]>(`${this.baseUrl}/admin/users`, {
        params: new HttpParams().set('role', 'DRIVER').set('limit', 500),
      })
      .subscribe({
  ...
  driverName(d: AdminUserOptionDto): string {
  ...
  load(): void {
    ...
    const url = this.selectedDriver
      ? `${this.baseUrl}/admin/drivers/${this.selectedDriver}/rides`
  ...
  private renderMiniMap(rideId: number): void {
    const el = document.getElementById(`admin-driver-ride-map-${rideId}`);
